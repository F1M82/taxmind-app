...
<title>TaxMind — Tax Intelligence</title>
...
.msg-row{display:flex;padding:4px 20px;gap:10px;animation:msgIn .18s ease}
...
.bubble{max-width:76%;padding:12px 16px;border-radius:11px;font-size:13px;line-height:1.76}
.bubble.ai{background:white;border:1px solid var(--border);color:var(--text);border-top-left-radius:3px;box-shadow:var(--shadow)}
...
<div class="mo" id="setupModal" style="display:none">
  <div class="mb">
    ...
    <button class="mb-btn" onclick="closeSetup()">Open Knowledge Base →</button>
  ...
  <div class="sidebar" id="sidebar">
    ...
    <div class="sidebar-scroll">
      <div class="sb-sect">
        ...
        <button class="sb-btn active" onclick="sw('chat')" id="sb-chat"><span class="ic">💬</span>Tax Query</button>
        <button class="sb-btn" onclick="sw('notice')" id="sb-notice"><span class="ic">📋</span>Notice Analyzer<span class="tag">KEY</span></button>
        <button class="sb-btn" onclick="sw('crossref')" id="sb-crossref"><span class="ic">🔀</span>Section Cross-Ref</button>
        <button class="sb-btn" onclick="sw('kb')" id="sb-kb"><span class="ic">📚</span>Knowledge Base</button>
      </div>
      <!-- KB Status -->
      <div class="kb-pill warn" id="kbPill">
        <div><span class="kp-dot"></span><span class="kp-t" id="kbPillTitle">Loading KB…</span></div>
    ...
    <div class="sidebar-footer"><p>⚖ ITA 2025 · Effective 1 Apr 2026<br>Shared Knowledge Base · Team Edition<br>Powered by Claude Sonnet 4</p></div>
  </div>
  ...
  <div class="main">
    ...
    <div class="topbar">
      ...
      <div class="tb-right">
        ...
        <span class="tb-label" id="kbLabel">Loading…</span>
      </div>
    </div>
    ...
    <div class="panel active" id="panel-chat">
      <div class="chat-area" id="chatArea">
        <div class="welcome" id="welcomeScr">
          ...
          <div class="kb-notice" id="kbNotice">⏳ Loading shared knowledge base…</div>
          <div class="chips" id="chips"></div>
        </div>
      </div>
      <div class="input-bar">
        <div class="input-wrap">
          <textarea class="input-ta" id="chatInput" rows="1" placeholder="Ask anything about Income Tax Act 2025…" onkeydown="hkey(event)" oninput="resize(this)"></textarea>
          <button class="send-btn" id="sendBtn" onclick="sendChat()" disabled>↑</button>
        </div>
        <div class="input-hint">Enter to send · Shift+Enter new line · Always cites new & old Act sections</div>
      </div>
    </div>
    ...
    <div class="panel" id="panel-notice">
      <div class="cpanel">
        <div class="ptitle">📋 Notice Analyzer & Draft Reply</div>
        ...
        <div class="icards">
          <div class="icard"><div class="ci">📄</div><div class="cl">Notice Types</div><div class="cv">143(1), 148, 148A, 154, 156, 263, 271 & all standard types</div></div>
          <div class="icard"><div class="ci">⚡</div><div class="cl">Output</div><div class="cv">Complete draft — grounds, legal submissions, case laws, prayer</div></div>
          <div class="icard"><div class="ci">🔒</div><div class="cl">Confidentiality</div><div class="cv">Replace client name & PAN with [CLIENT NAME] and [PAN]</div></div>
        </div>
        ...
        <div class="arow">
          <button class="pbtn" id="analyzeBtn" onclick="analyzeNotice()">⚡ Analyze & Draft Reply</button>
          <button class="gbtn" onclick="document.getElementById('noticeInput').value=''">Clear</button>
        </div>
        <div class="warn-note">⚠ Replace actual client name and PAN before pasting.</div>
        <div id="noticeResult" style="margin-top:20px;display:none">
          ...
          <div style="background:white;border:1px solid var(--border);border-radius:9px;padding:18px;box-shadow:var(--shadow)">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
              <div style="font-weight:600;font-size:13.5px;color:var(--ink)">📝 Draft Reply</div>
    ...
    <div class="panel" id="panel-crossref">
      <div class="cpanel">
        <div class="ptitle">🔀 Section Cross-Reference</div>
        ...
        <button class="pbtn" onclick="lookup()">Look Up →</button>
    ...
    <div class="panel" id="panel-kb">
      <div class="cpanel">
        ...
        <div class="psub">Upload documents below — they're shared with your entire team instantly. Stored centrally; no one needs to upload individually.</div>
        <div class="kstats">
          <div class="scard"><div class="sn" id="stDocs">—</div><div class="sl">Shared Docs</div></div>
          <div class="scard"><div class="sn" id="stChunks">—</div><div class="sl">Text Chunks</div></div>
          <div class="scard"><div class="sn" id="stLocal">0</div><div class="sl">Local Only</div></div>
          <div class="scard"><div class="sn" id="stSize">—</div><div class="sl">Storage</div></div>
        </div>

        <!-- Sync bar -->
        <div class="sync-bar">
          <div class="sb-info">
            <div class="sb-title">🔄 Shared Knowledge Base</div>
        ...
        <div class="upload-form">
          <div style="font-size:13.5px;font-weight:600;color:var(--ink);margin-bottom:13px">📤 Add Document (shared with team)</div>
          <div class="fgrid">
            ...
            <div class="fg" id="clOut" style="display:none"><label>Outcome</label>
              <select class="fs" id="docOut">
                ...
                <option value="taxpayer_favorable">Taxpayer Favorable ✓</option>
          ...
          <button class="pbtn" style="margin-top:12px;width:100%" id="idxBtn" onclick="indexDoc()">📤 Index & Share with Team</button>
        </div>
        ...
        <div class="doc-list" id="docList"><div class="empty-st"><div class="esi">📭</div>No documents yet. Upload your ITA 2025 PDF above.</div></div>
      </div>
    </div>
  </div>
</div>
...
const QQ=['What replaces Section 80C in the new Act?','Compare Section 148 reassessment old vs new','New TDS rates under ITA 2025','Capital gains changes in new Act','Section 68 unexplained cash credit — what changed?','New presumptive taxation — 44AD equivalent'];
const SECS=['80C','80D','10(38)','148','148A','147','263','68','69A','54','54F','24(b)','44AD','44ADA','115BAC','271AAB'];

// ── INIT ───────────────────────────────────────────────────
document.addEventListener('DOMContentLoaded',()=>{
  renderQQ(); renderPills();
  syncKB(true); // silent initial load
});
...
async function syncKB(silent=false){
  ...
  try{
    ...
    // Server returns metadata only — we need full chunks for retrieval
    // Full chunks are stored in sharedKB after indexing from this device
    // On other devices: chunks come from server if stored there, or we show "loaded from server"
    if(data.documents){
      // Merge: prefer server data
      sharedKB=data.documents;
      updateKBUI();
      if(!silent) showToast(`Synced — ${sharedKB.length} shared documents`,'success');
    }
  }catch(e){
...
async function pushToServer(docs){
  try{
    // Split chunks into batches of 50 to stay under Upstash 1MB request limit
    for(const doc of docs){
      const BATCH=50;
      const allChunks=doc.chunks||[];
      const batches=[];
      for(let i=0;i<allChunks.length;i+=BATCH) batches.push(allChunks.slice(i,i+BATCH));
      
      for(let b=0;b<batches.length;b++){
        const partial={...doc, chunks:batches[b], chunkBatch:b, totalBatches:batches.length};
        document.getElementById('progLbl').textContent=`Uploading batch ${b+1}/${batches.length}…`;
        const res=await fetch('/api/kb',{
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body:JSON.stringify({documents:[partial]})
        });
        if(!res.ok) throw new Error('Upload failed on batch '+(b+1));
        const data=await res.json();
        if(!data.ok) throw new Error(data.error||'Server rejected batch '+(b+1));
      }
    }
    return true;
  }catch(e){
...
function updateKBUI(){
  ...
  document.getElementById('stChunks').textContent=totalChunks.toLocaleString()||'—';
  ...
  if(total>0){
    ...
    if(notice) notice.textContent=`✅ ${total} document${total>1?'s':''} loaded — answers grounded in your materials`;
    ...
    lbl.textContent='KB empty — upload docs';
    if(notice) notice.textContent='⚠️ Knowledge base empty — go to Knowledge Base tab to upload ITA 2025 PDF';
  }
...
function renderDocList(){
  ...
  if(!all.length){list.innerHTML='<div class="empty-st"><div class="esi">📭</div>No documents yet. Upload your ITA 2025 PDF above.</div>';return;}
  const ic={'New Act':'📗','Old Act':'📘','Case Law':'⚖️','ITAT Order':'🏛','CBDT Circular':'📜','Section Mapping':'🔀','Internal Notes':'📝'};
  list.innerHTML=all.map(d=>`
    <div class="drow">
      <div class="di">${ic[d.category]||'📄'}</div>
      <div class="dinfo">
...
async function extractPDF(file){
  ...
  for(let i=1;i<=pdf.numPages;i++){
    ...
    document.getElementById('progLbl').textContent=`Extracting page ${i} of ${pdf.numPages}…`;
  }
  return text;
}
...
function chunk(text){
  ...
  for(const x of s){if(len+x.length>3200&&cur.length){out.push(cur.join(' '));cur=cur.slice(-2);len=cur.reduce((a,c)=>a+c.length,0);}cur.push(x);len+=x.length;}
...
async function indexDoc(){
  ...
  if(selFile.size>25*1024*1024){showToast('File too large — max 25MB','error');return;}
  ...
  document.getElementById('progLbl').textContent='Reading…';
  try{
    let text=selFile.name.toLowerCase().endsWith('.pdf')?await extractPDF(selFile):await selFile.text();
    document.getElementById('progLbl').textContent='Chunking…';
    document.getElementById('pFill').style.width='80%';
    const chunks=chunk(text);
    document.getElementById('progLbl').textContent='Uploading to shared KB…';
    ...
    if(ok){
      ...
      document.getElementById('progLbl').textContent=`✅ Shared with team — ${chunks.length} chunks`;
      showToast(`✅ "${title}" shared with team!`,'success');
    }else{
      // Fallback: local only
      localKB.push(doc); saveLocal();
      document.getElementById('progLbl').textContent=`✅ Saved locally — ${chunks.length} chunks`;
...
function embed(t){
  const f={};
  t.toLowerCase().replace(/[^a-z0-9\s]/g,' ').split(/\s+/).filter(Boolean).forEach(w=>{f[w]=(f[w]||0)+1;});
  return['section','income','tax','act','deduction','exemption','capital','gain','tds','return','assessment','appeal','penalty','interest','reassessment','notice','reply','order','tribunal','court','judgment','case','provision','limit','rate','new','old','amendment','circular','cbdt','rule','salary','business','property','investment','benefit','claim','taxpayer','assessee','officer','commissioner','addition','disallowance','unexplained','cash','credit','deposit','purchase','evidence','proof','finance','notification','search','survey','dividend','royalty','fees'].map(w=>f[w]||0);
}
function cos(a,b){let d=0,na=0,nb=0;for(let i=0;i<a.length;i++){d+=a[i]*b[i];na+=a[i]*a[i];nb+=b[i]*b[i];}return d/(Math.sqrt(na)*Math.sqrt(nb)+1e-8);}

function getCtx(q,k=6){
  const all=KB();
  if(!all.length)return'';
  const qe=embed(q);
  const sc=[];
  all.forEach(doc=>(doc.chunks||[]).forEach(c=>sc.push({c,doc,s:cos(qe,embed(c))})));
  sc.sort((a,b)=>b.s-a.s);
  return sc.slice(0,k).map((r,i)=>`[${i+1}] ${r.doc.title} (${r.doc.category})\n${r.c}`).join('\n\n---\n\n');
}

// ── PROMPTS ────────────────────────────────────────────────
const SYS_TAX=`You are TaxMind — Tax Intelligence Assistant for expert Indian tax & finance professionals. Expertise in both ITA 1961 and ITA 2025 (effective 1 Apr 2026).
MANDATORY: 1) Always cite BOTH new Act 2025 section AND old Act 1961 equivalent. 2) State what changed/added/removed. 3) Cite CBDT circulars with number+date. 4) Integrate case laws: "The Hon'ble [Court] in [Case] [Citation] held that [ratio]." 5) Never invent section numbers. 6) End with ## References section listing all sources used.
...
// ── CLAUDE CALL (via secure proxy) ────────────────────────
async function claude(sys, messages, kbctx='', webctx='') {
  const s = sys
  ...
  const r = await fetch('/api/claude', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ model: MODEL, max_tokens: 3000, system: s, messages })
  });
...
async function sendChat(){
  ...
  const tid=showTyping('Searching KB + web…');
  try{
    // Run KB retrieval and web search in parallel
    const [kbctx, webData] = await Promise.all([
      Promise.resolve(getCtx(text)),
      webSearch(text + ' income tax India 2025')
    ]);
    const webctx = formatWebResults(webData.results);
    const sources = [];
    if(KB().length) sources.push('📚 Knowledge Base');
    if(webData.results.length) sources.push('🌐 Web ('+webData.results.length+' results)');

    rmTyping(tid);
    const tid2=showTyping('Generating answer…');
    const reply=await claude(SYS_TAX,msgs,kbctx,webctx);
    msgs.push({role:'assistant',content:reply});
    rmTyping(tid2);
    addMsg('assistant',reply,sources);
  }catch(e){rmTyping(tid);addMsg('assistant','⚠️ '+e.message);showToast(e.message,'error');}
}
...
function addMsg(role,content,srcs=[]){
  ...
  row.innerHTML=`<div class="av ${role==='assistant'?'ai':'user'}">${role==='assistant'?'⚖':'👤'}</div><div><div class="bubble ${role==='assistant'?'ai':'user'}">${b}</div>${s}<div class="b-meta">${now}</div></div>`;
...
function showTyping(label=''){
  ...
  r.innerHTML=`<div class="av ai">⚖</div><div style="display:flex;align-items:center"><div class="typing"><div class="dot"></div><div class="dot"></div><div class="dot"></div></div>${lbl}</div>`;
...
async function analyzeNotice(){
  ...
  try{
    ...
    if(sm){
      ...
      document.getElementById('noticeSumCard').innerHTML=`<h3>📋 Notice Summary ${urgent?'<span class="ubadge">⚠ URGENT</span>':''}</h3><div class="nsg">${items.map(i=>`<div class="nsi"><div class="nsk">${i.k}</div><div class="nsv">${i.v}</div></div>`).join('')}</div>`;
    }
...
async function lookup(){
  ...
  document.getElementById('crContent').innerHTML='<p style="color:var(--text-mid)">Searching KB and web…</p>';
